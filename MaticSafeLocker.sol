...
contract MaticSafeLocker is Ownable, ReentrancyGuard {
  ...
  IQuickFactory public quickFactory;
  ...
  mapping(address => TokenLock[]) public tokenLocks; //map UNI-V2 pair to all its locks
  
  struct FeeStruct {
    uint256 maticFee; // Small matic fee to prevent spam on the platform
    IERCBurn secondaryFeeToken; // SAFE
    uint256 secondaryTokenFee; // optional, SAFE
    uint256 secondaryTokenDiscount; // discount on liquidity fee for burning secondaryToken
    uint256 liquidityFee; // fee on UNI-V2 liquidity tokens
  ...
  constructor(IQuickFactory _quickFactory) public {
    devaddr = msg.sender;
    gFees.referralPercent = 250; // 25%
    gFees.maticFee = 1000e18;
  ...
  function setDev(address payable _devaddr) public onlyOwner {
    ...
    /**
   * @notice set the migrator contract which allows locked lp tokens to be migrated to QuickSwap V3
   */
  ...
  function setFees(uint256 _referralPercent, uint256 _referralDiscount, uint256 _maticFee, uint256 _secondaryTokenFee, uint256 _secondaryTokenDiscount, uint256 _liquidityFee) public onlyOwner {
    gFees.referralPercent = _referralPercent;
    gFees.referralDiscount = _referralDiscount;
    gFees.maticFee = _maticFee;
   ...
   * @param _lpToken the UNI-V2 token address
   * @param _amount amount of LP tokens to lock
   * @param _unlock_date the unix timestamp (in seconds) until unlock
   * @param _referral the referrer address if any or address(0) for none
   * @param _fee_in_matic fees can be paid in matic or in a secondary token such as SAFE with a discount on UNI-V2 tokens
   * @param _withdrawer the user who can withdraw liquidity once the lock expires.
   */
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_matic, address payable _withdrawer) external payable nonReentrant {
    require(_unlock_date < 10000000000, 'TIMESTAMP INVALID'); // prevents errors when timestamp entered in milliseconds
    require(_amount > 0, 'INSUFFICIENT');

    // ensure this pair is a quick pair by querying the factory
    IQuickPair lpair = IQuickPair(address(_lpToken));
    address factoryPairAddress = quickFactory.getPair(lpair.token0(), lpair.token1());
    require(factoryPairAddress == address(_lpToken), 'NOT UNI-V2');
    ...
    if (!feeWhitelist.contains(msg.sender)) {
      if (_fee_in_matic) { // charge fee in matic
        uint256 maticFee = gFees.maticFee;
        if (_referral != address(0)) {
          maticFee = maticFee.mul(1000 - gFees.referralDiscount).div(1000);
        }
        require(msg.value == maticFee, 'FEE NOT MET');
        uint256 devFee = maticFee;
        if (maticFee != 0 && _referral != address(0)) { // referral fee
      ...
      // refund matic if a whitelisted member sent it by mistake
      msg.sender.transfer(msg.value);
    }
    
    // percent fee
    uint256 liquidityFee = _amount.mul(gFees.liquidityFee).div(1000);
    if (!_fee_in_matic && !feeWhitelist.contains(msg.sender)) { // fee discount for large lockers using secondary token
    ...
    // record the lock for the UNI-V2
  ...
  function splitLock (address _lpToken, uint256 _index, uint256 _lockID, uint256 _amount) external payable nonReentrant {
    ...
    require(msg.value == gFees.maticFee, 'FEE NOT MET');
    devaddr.transfer(gFees.maticFee);
    ...
    // record the lock for the UNI-V2 pair
    tokenLocks[_lpToken].push(token_lock);
  ...
  /**
   * @notice migrates liquidity to QuickSwap V3
   */
