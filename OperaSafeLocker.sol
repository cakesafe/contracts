...
contract OperaSafeLocker is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;

  ISpiritFactory public spiritFactory;
  ...
  mapping(address => TokenLock[]) public tokenLocks; //map SPIRT-LP pair to all its locks
  ...
  struct FeeStruct {
    ...
    uint256 liquidityFee; // fee on SPIRT-LP liquidity tokens
  ...
  constructor(ISpiritFactory _spiritFactory) public {
    devaddr = msg.sender;
  ...
  function setDev(address payable _devaddr) public onlyOwner {
    ...
    /**
   * @notice set the migrator contract which allows locked lp tokens to be migrated to SpiritSwap V3
   */
   ...
   * @param _lpToken the SPIRT-LP token address
   * @param _amount amount of LP tokens to lock
   * @param _unlock_date the unix timestamp (in seconds) until unlock
   * @param _referral the referrer address if any or address(0) for none
   * @param _fee_in_ftm fees can be paid in ftm or in a secondary token such as SAFE with a discount on SPIRT-LP tokens
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_ftm, address payable _withdrawer) external payable nonReentrant {
    ...
    // ensure this pair is a spirit pair by querying the factory
    ISpiritPair lpair = ISpiritPair(address(_lpToken));
    address factoryPairAddress = spiritFactory.getPair(lpair.token0(), lpair.token1());
    require(factoryPairAddress == address(_lpToken), 'NOT SPIRT-LP');
    ...
    // record the lock for the SPIRT-LP
  ...
  function splitLock (address _lpToken, uint256 _index, uint256 _lockID, uint256 _amount) external payable nonReentrant {
    ...
    // record the lock for the SPIRT-LP pair
    tokenLocks[_lpToken].push(token_lock);
  ...
  /**
   * @notice migrates liquidity to SpiritSwap V2
   */
